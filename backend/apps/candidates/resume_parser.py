...
Extract structured data from the raw resume text and return only valid JSON.
Do not include markdown, commentary, or fields outside the requested structure.
Do not fabricate details that are not explicitly stated or clearly implied.
Every skill, employer, school, certification, URL, email, and phone value must be present
in the raw resume text. If a value is uncertain or not visible, use null or [].
...
class Skill(StrictModel):
    ...
    def normalize_proficiency(cls, value: str | None) -> Proficiency:
        ...
        return normalized if normalized in allowed else "beginner"
...
KNOWN_SKILLS: list[tuple[str, SkillCategory, tuple[str, ...]]] = [
    ...
    ("Tailwind CSS", "framework", ("tailwind css", "tailwind")),
...
def llm_parse_models() -> list[str]:
    configured = [
        getattr(settings, "LLM_MODEL", "qwen2.5-coder:7b"),
...
def request_ollama_parse(raw_text: str, model: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    for num_predict in ollama_num_predict_attempts():
        ...
        try:
            parsed = parse_json_object(body.get("response", ""))
...
def infer_skill_models(raw_text: str) -> list[Skill]:
    ...
    for canonical, category, aliases in KNOWN_SKILLS:
        if any(alias_supported(alias, raw_text) for alias in aliases):
            skills.append(Skill(name=canonical, proficiency="beginner", category=category))
    return skills
...
def build_heuristic_parse(raw_text: str, reason: str) -> ParsedResumeSchema:
    ...
    skills = [
        Skill(name=name, proficiency="beginner", category="other")
