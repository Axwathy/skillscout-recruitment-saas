...
from apps.ai_engine.tasks import generate_job_embedding, generate_parsed_resume_embedding
...
class RankedCandidatesView(views.APIView):
    ...
    def get(self, request, pk, *args, **kwargs):
        ...
        ranked_scores = rank_candidates_for_job(job, force=force)[:limit]
        results = []
        ...
        AuditLog.log(
            ...
            entity=job,
            ip_address=request.META.get("REMOTE_ADDR"),
            metadata={"candidate_count": len(results), "force": force},
        )
        return Response(
            {
                "job_id": str(job.id),
